...
impl Template {
    ...
    pub fn reassign_languages(&mut self, handle: usize, languages: Vec<String>) -> Option<Vec<String>> {
        match self.handle_to_index.get(&handle) {
            Some(_) => {
...
mod tests {
    mod template {
        ...
        fn reassign_languages() {
            let mut template = empty_template();
            let handle_1 = template.add_content("foo bar".to_string(), vec!["1".to_owned(), "2".to_owned()]);
            let handle_2 = template.add_content("foo bar".to_string(), vec!["3".to_owned(), "4".to_owned()]);
            let old_languages = template.reassign_languages(handle_1, vec!["2".to_owned(), "5".to_owned()]);

            assert!(old_languages.is_some());
            let Some(mut languages) = old_languages else { panic!() };
            languages.sort();
            assert_eq!(languages, vec!["1".to_owned(), "2".to_owned()]);
        ...
        fn reassign_not_existing_languages() {
            let mut template = empty_template();
            let handle_1 = template.add_content("foo bar".to_string(), vec!["1".to_owned(), "2".to_owned()]);
            let _handle_2 = template.add_content("foo bar".to_string(), vec!["3".to_owned(), "4".to_owned()]);
            let old_languages = template.reassign_languages(handle_1 + 100, vec!["2".to_owned(), "5".to_owned()]);
